...
| [EXPRESSION_FIELD_SEAMS](./EXPRESSION_FIELD_SEAMS.md) | done pending user verify (uncommitted, branch `bugfix/116_expression-field-seams`) — prod tsc / lint / unit 919/919 / ExpressionField stories 8/8 green, new-story PNG read | **12th (bugfix).** The `extraActions` seam stopped one hop short of the `ExpressionField` shell hosts mount, and the builder's operand `TemplateField` mounts dropped it again — qorus-ide lost its AI-assist button on every expression card when it adopted the shell ([#116](https://github.com/qoretechnologies/toolkit-react/issues/116)). Forward it through both hops; pin with a nested-operand story; ships as a per-PR patch (`0.10.51`; re-based onto `4860e6e` after #114 published 0.10.50). |
